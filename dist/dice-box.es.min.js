var Zl = Object.defineProperty;
var ml = (c, b, l) => b in c ? Zl(c, b, { enumerable: !0, configurable: !0, writable: !0, value: l }) : c[b] = l;
var N = (c, b, l) => (ml(c, typeof b != "symbol" ? b + "" : b, l), l), E = (c, b, l) => {
  if (!b.has(c))
    throw TypeError("Cannot " + l);
};
var V = (c, b, l) => (E(c, b, "read from private field"), l ? l.call(c) : b.get(c)), p = (c, b, l) => {
  if (b.has(c))
    throw TypeError("Cannot add the same private member more than once");
  b instanceof WeakSet ? b.add(c) : b.set(c, l);
}, h = (c, b, l, d) => (E(c, b, "write to private field"), d ? d.call(c, l) : b.set(c, l), l), S = (c, b, l, d) => ({
  set _(X) {
    h(c, b, X, l);
  },
  get _() {
    return V(c, b, d);
  }
}), s = (c, b, l) => (E(c, b, "access private method"), l);
function cl(c) {
  const { selector: b, id: l } = c;
  if (!b || typeof b != "string")
    throw new Error("You must provide a DOM selector as the first argument in order to render the Dice Box");
  let d;
  const X = document.querySelector(b);
  if (!(X != null && X.nodeName))
    throw new Error(`DiceBox target DOM node: '${b}' not found or not available yet. Try invoking inside a DOMContentLoaded event`);
  return X.nodeName.toLowerCase() !== "canvas" ? (d = document.createElement("canvas"), d.id = l, X.appendChild(d)) : d = X, d;
}
const $ = "", O = typeof window < "u" && window.Blob && new Blob([atob($)], { type: "text/javascript;charset=utf-8" });
function Gl() {
  const c = O && (window.URL || window.webkitURL).createObjectURL(O);
  try {
    return c ? new Worker(c) : new Worker("data:application/javascript;base64," + $);
  } finally {
    c && (window.URL || window.webkitURL).revokeObjectURL(c);
  }
}
const Wl = (c = { dedupe: !1 }) => {
  const { dedupe: b } = c;
  let l = [], d;
  const X = (m) => (b && (l = []), l.push(m), d || (d = Z()), d.finally(() => {
    d = void 0;
  })), Z = async () => {
    const m = [];
    for (; l.length; ) {
      const W = l.shift();
      m.push(await W());
    }
    return m;
  };
  return { push: X, queue: l, flush: () => d || Promise.resolve([]) };
}, Yl = (c) => JSON.parse(JSON.stringify(c));
class q {
  static value() {
    const b = window.crypto || window.msCrypto, l = new Uint32Array(1);
    return b.getRandomValues(l)[0] / 2 ** 32;
  }
  static range(b, l) {
    return Math.floor(Math.pow(10, 14) * this.value() * this.value()) % (l - b + 1) + b;
  }
}
const Vl = (c) => {
  let b = !1, l = c.slice(c.startsWith("#") ? 1 : 0);
  l.length === 3 ? l = [...l].map((X) => X + X).join("") : l.length === 8 && (b = !0), l = parseInt(l, 16);
  let d = { r: l >>> 16, g: (l & 65280) >>> 8, b: l & 255 };
  return b && (d.r = l >>> 24, d.g = (l & 16711680) >>> 16, d.b = (l & 65280) >>> 8, d.a = l & 255), d;
}, Rl = { id: `dice-canvas-${Date.now()}`, enableShadows: !0, shadowTransparency: 0.8, lightIntensity: 1, delay: 10, scale: 5, theme: "default", themeColor: "#2e8555", offscreen: !0, assetPath: "/assets/dice-box/", origin: location.origin, meshFile: "models/default.json", suspendSimulation: !1 };
var H, o, f, x, u, K, e, a, z, B, k, _, Q, ll, P, bl, t, g, w, M;
class Jl {
  constructor(b, l = {}) {
    p(this, k);
    p(this, Q);
    p(this, P);
    p(this, t);
    p(this, w);
    N(this, "rollCollectionData", {});
    N(this, "rollGroupData", {});
    N(this, "rollDiceData", {});
    N(this, "themeData", []);
    N(this, "themesLoadedData", {});
    p(this, H, 0);
    p(this, o, 0);
    p(this, f, 0);
    p(this, x, 0);
    p(this, u, {});
    p(this, K, void 0);
    p(this, e, void 0);
    p(this, a, void 0);
    p(this, z, void 0);
    p(this, B, void 0);
    N(this, "noop", () => {
    });
    if (typeof l != "object")
      throw new Error("Config options should be an object. Config reference: https://fantasticdice.games/docs/usage/config#configuration-options");
    const { onDieComplete: d, onRollComplete: X, onRemoveComplete: Z, onThemeConfigLoaded: m, onThemeLoaded: W, ...G } = l;
    this.config = { ...Rl, ...G }, this.onDieComplete = l.onDieComplete || this.noop, this.onRollComplete = l.onRollComplete || this.noop, this.onRemoveComplete = l.onRemoveComplete || this.noop, this.onThemeLoaded = l.onThemeLoaded || this.noop, this.onThemeConfigLoaded = l.onThemeConfigLoaded || this.noop, this.canvas = cl({ selector: b, id: this.config.id }), this.loadThemeQueue = Wl({ dedupe: !0 });
  }
  resizeWorld(b) {
    V(this, u).resize(b), V(this, a).postMessage({ action: "resize", ...b });
  }
  async init() {
    return s(this, Q, ll).call(this), await s(this, k, _).call(this), V(this, u).onRollResult = (b) => {
      const l = this.rollDiceData[b.rollId], d = this.rollGroupData[l.groupId], X = this.rollCollectionData[l.collectionId];
      d.rolls[l.rollId].value = b.value, X.completedRolls++, X.completedRolls == X.rolls.length && X.resolve(Object.values(X.rolls).map(({ collectionId: G, id: Y, meshName: R, ...y }) => y));
      const { collectionId: Z, id: m, ...W } = l;
      this.onDieComplete(W);
    }, V(this, u).onRollComplete = () => {
      this.onRollComplete(this.getRollResults());
    }, V(this, u).onDieRemoved = (b) => {
      let l = this.rollDiceData[b];
      const d = this.rollCollectionData[l.removeCollectionId];
      d.completedRolls++, delete this.rollDiceData[l.rollId];
      const X = this.rollGroupData[l.groupId];
      delete X.rolls[l.rollId];
      const Z = s(this, w, M).call(this, l.groupId);
      X.value = Z.value, X.qty = Z.rollsArray.length, d.completedRolls == d.rolls.length && d.resolve(Object.values(d.rolls).map(({ id: y, ...L }) => L));
      const { collectionId: m, id: W, removeCollectionId: G, meshName: Y, ...R } = l;
      this.onRemoveComplete(R);
    }, await Promise.all([V(this, K), V(this, z)]), s(this, P, bl).call(this), await this.loadThemeQueue.push(() => this.loadTheme(this.config.theme)), this;
  }
  async getThemeConfig(b) {
    const l = `${this.config.origin}${this.config.assetPath}themes/${b}`;
    let d;
    b === "default" ? d = { name: "Default Colors", material: { type: "color", diffuseTexture: { light: "diffuse-light.png", dark: "diffuse-dark.png" }, diffuseLevel: 1, bumpTexture: "normal.png", bumpLevel: 0.5, specularTexture: "specular.jpg", specularPower: 1 } } : d = await fetch(`${l}/theme.config.json`).then((m) => {
      if (m.ok) {
        const W = m.headers.get("content-type");
        if (W && W.indexOf("application/json") !== -1 || m.type && m.type === "basic")
          return m.json();
        throw new Error(`Incorrect contentType: ${W}. Expected "application/json" or "basic"`);
      } else
        throw new Error(`Unable to fetch config file for theme: '${b}'. Request rejected with status ${m.status}: ${m.statusText}`);
    }).catch((m) => console.error(m));
    let X = this.config.origin + this.config.assetPath + this.config.meshFile, Z = "default";
    if (!d)
      throw new Error("No theme config data to work with.");
    if (d.hasOwnProperty("meshFile") && (X = `${l}/${d.meshFile}`, d.hasOwnProperty("meshName") ? Z = d.meshName : (console.warn("You should provide a meshName in your theme.config.json file"), Z = d.meshFile.replace(/(.*)\..{2,4}$/, "$1"))), d.hasOwnProperty("diceAvailable") || (d.diceAvailable = ["d4", "d6", "d8", "d10", "d12", "d20", "d100"]), d.hasOwnProperty("extends")) {
      let m = this.themesLoadedData[d.extends];
      m || (m = await this.loadTheme(d.extends).catch((W) => console.error(W))), m && (d.diceInherited = [...d.diceInherited || []], m.diceAvailable.map((W) => {
        d.diceInherited[W] = m.systemName;
      }));
    }
    return Object.assign(d, { basePath: l, meshFilePath: X, meshName: Z, theme: b }), d;
  }
  async loadTheme(b) {
    if (this.themesLoadedData[b])
      return this.themesLoadedData[b];
    const l = this.themesLoadedData[b] = await this.getThemeConfig(b).catch((d) => console.error(d));
    if (this.onThemeConfigLoaded(l), !!l)
      return await V(this, u).loadTheme(l).catch((d) => console.error(d)), this.onThemeLoaded(l), l;
  }
  async updateConfig(b) {
    const l = { ...this.config, ...b };
    return await this.loadThemeQueue.push(() => this.loadTheme(l.theme)), this.config = l, V(this, u).updateConfig(l), V(this, a).postMessage({ action: "updateConfig", options: l }), this;
  }
  clear() {
    return h(this, H, 0), h(this, o, 0), h(this, f, 0), h(this, x, 0), this.rollCollectionData = {}, this.rollGroupData = {}, this.rollDiceData = {}, V(this, u).clear(), V(this, a).postMessage({ action: "clearDice" }), this;
  }
  hide() {
    return this.canvas.style.display = "none", this;
  }
  show() {
    return this.canvas.style.display = "block", this.resizeWorld({ width: this.canvas.clientWidth, height: this.canvas.clientHeight }), this;
  }
  roll(b, { theme: l = this.config.theme, themeColor: d = this.config.themeColor, newStartPoint: X = !0 } = {}) {
    this.clear();
    const Z = S(this, H)._++;
    this.rollCollectionData[Z] = new T({ id: Z, notation: b, theme: l, themeColor: d, newStartPoint: X });
    const m = this.createNotationArray(b, this.themesLoadedData[l].diceAvailable);
    return s(this, t, g).call(this, m, Z), this.rollCollectionData[Z].promise;
  }
  add(b, { theme: l = this.config.theme, themeColor: d = this.config.themeColor, newStartPoint: X = !0 } = {}) {
    const Z = S(this, H)._++;
    this.rollCollectionData[Z] = new T({ id: Z, notation: b, theme: l, themeColor: d, newStartPoint: X });
    const m = this.createNotationArray(b, this.themesLoadedData[l].diceAvailable);
    return s(this, t, g).call(this, m, Z), this.rollCollectionData[Z].promise;
  }
  reroll(b, { remove: l = !1, hide: d = !1, newStartPoint: X = !0 } = {}) {
    const Z = (Array.isArray(b) ? b : [b]).map(({ value: m, ...W }) => W);
    return l === !0 && this.remove(Z, { hide: d }), this.add(Z, { newStartPoint: X });
  }
  remove(b, { hide: l = !1 } = {}) {
    const d = Array.isArray(b) ? b : [b], X = S(this, H)._++;
    return this.rollCollectionData[X] = new T({ id: X, notation: b, rolls: d }), d.map((Z) => {
      this.rollDiceData[Z.rollId].removeCollectionId = X;
      let m = this.rollDiceData[Z.rollId].id;
      V(this, u).remove({ id: m, rollId: Z.rollId }), V(this, a).postMessage({ action: "removeDie", id: m });
    }), this.rollCollectionData[X].promise;
  }
  createNotationArray(b, l) {
    const d = Array.isArray(b) ? b : [b];
    let X = [];
    const Z = (G) => {
      if (G.hasOwnProperty("qty") || (G.qty = 1), G.hasOwnProperty("sides"))
        return !0;
      {
        const Y = "Roll notation is missing sides";
        throw new Error(Y);
      }
    }, m = (G) => {
      G = G.toString();
      let Y = G.split(".");
      return Y[1] ? Y[1] = parseInt(Y[1]) + 1 : Y[1] = 1, Y[0] + "." + Y[1];
    }, W = (G) => {
      G.hasOwnProperty("rollId") && this.rollDiceData.hasOwnProperty(G.rollId) && (G.rollId = m(G.rollId));
    };
    return d.forEach((G) => {
      typeof G == "string" ? X.push(this.parse(G, l)) : typeof d == "object" && (W(G), Z(G) && X.push(G));
    }), X;
  }
  parse(b, l) {
    const d = /(\d+)([dD]{1}\d+)(.*)$/i, X = /(\d+)[dD]([0%]+)(.*)$/i, Z = /(\d+)[dD](f+[ate]*)(.*)$/i, m = /(\d+)[dD]([\d\w]+)([+-]{0,1}\d+)?/i, W = /([+-])(\d+)/, G = b.trim().replace(/\s+/g, ""), Y = (i, F) => {
      if (i = Number(i), Number.isNaN(i) || !Number.isInteger(i) || i < 1)
        throw new Error(F);
      return i;
    }, R = G.match(X) || G.match(d) || G.match(Z) || G.match(m);
    let y = 0;
    const L = "Invalid notation: " + b;
    if (!R || !R.length || R.length < 3)
      throw new Error(L);
    if (R[3] && W.test(R[3])) {
      const i = R[3].match(W);
      let F = Y(i[2], L);
      i[1].trim() === "-" && (F *= -1), y = F;
    }
    const v = { qty: Y(R[1], L), modifier: y };
    return G.match(X) ? v.sides = "100" : G.match(Z) ? v.sides = "fate" : (l.includes(G.match(m)[2]), v.sides = R[2]), v;
  }
  getRollResults() {
    return Object.entries(this.rollGroupData).map(([b, l]) => {
      const d = s(this, w, M).call(this, b);
      l.value = d.value, l.qty = d.rollsArray.length;
      const X = { ...l };
      return X.rolls = d.rollsArray, X;
    });
  }
}
H = new WeakMap(), o = new WeakMap(), f = new WeakMap(), x = new WeakMap(), u = new WeakMap(), K = new WeakMap(), e = new WeakMap(), a = new WeakMap(), z = new WeakMap(), B = new WeakMap(), k = new WeakSet(), _ = async function() {
  h(this, K, new Promise((l, d) => {
    h(this, e, l);
  }));
  const b = () => {
    V(this, e).call(this);
  };
  if ("OffscreenCanvas" in window && "transferControlToOffscreen" in this.canvas && this.config.offscreen) {
    const l = await import("./world.offscreen.min.js").then((d) => d.default);
    h(this, u, new l({ canvas: this.canvas, options: this.config, onInitComplete: b }));
  } else {
    this.config.offscreen && (console.warn("This browser does not support OffscreenCanvas. Using standard canvas fallback."), this.config.offscreen = !1);
    const l = await import("./world.onscreen.min.js").then((d) => d.default);
    h(this, u, new l({ canvas: this.canvas, options: this.config, onInitComplete: b }));
  }
}, Q = new WeakSet(), ll = function() {
  h(this, a, new Gl()), h(this, z, new Promise((b, l) => {
    h(this, B, b);
  })), V(this, a).onmessage = (b) => {
    switch (b.data.action) {
      case "init-complete":
        V(this, B).call(this);
    }
  }, V(this, a).postMessage({ action: "init", width: this.canvas.clientWidth, height: this.canvas.clientHeight, options: this.config });
}, P = new WeakSet(), bl = function() {
  const b = new MessageChannel();
  V(this, u).connect(b.port1), V(this, a).postMessage({ action: "connect" }, [b.port2]);
}, t = new WeakSet(), g = async function(b, l) {
  const d = this.rollCollectionData[l];
  let X = d.newStartPoint;
  b.forEach(async (Z) => {
    if (!Z.sides)
      throw new Error("Improper dice notation or unable to parse notation");
    const m = Z.theme || d.theme || this.config.theme, W = Z.themeColor || d.themeColor || this.config.themeColor, G = {}, Y = Z.groupId !== void 0;
    let R;
    const y = () => this.loadTheme(m);
    await this.loadThemeQueue.push(y);
    const { meshName: L, diceAvailable: v, diceInherited: i = {}, material: { type: F } } = this.themesLoadedData[m], j = Object.keys(i);
    let D = "", C;
    F === "color" && (C = Vl(W), D = C.r * 0.299 + C.g * 0.587 + C.b * 0.114 > 175 ? "_dark" : "_light");
    for (var r = 0, dl = Z.qty; r < dl; r++) {
      let U = Z.rollId !== void 0 ? Z.rollId : S(this, f)._++, Xl = Z.id !== void 0 ? Z.id : S(this, x)._++;
      R = Y ? Z.groupId : V(this, o);
      const A = Number.isInteger(Z.sides) ? `d${Z.sides}` : Z.sides;
      Z.sides = A;
      const J = { sides: A, groupId: R, collectionId: d.id, rollId: U, id: Xl, theme: m, themeColor: W, meshName: L };
      if (G[U] = J, this.rollDiceData[U] = J, d.rolls.push(this.rollDiceData[U]), J.sides === "fate" && !v.includes(J.sides) && !j.includes(J.sides)) {
        console.warn(`fate die unavailable in '${m}' theme. Using fallback.`);
        const n = -1, I = 1;
        J.value = q.range(n, I), V(this, u).addNonDie(J);
      } else if (this.config.suspendSimulation || !v.includes(J.sides) && !j.includes(J.sides))
        console.warn(this.config.suspendSimulation ? "3D simulation suspended. Using fallback." : `${J.sides} die unavailable in '${m}' theme. Using fallback.`), J.value = q.range(1, J.sides), V(this, u).addNonDie(J);
      else {
        let n;
        if (j.includes(J.sides)) {
          const I = i[J.sides];
          n = this.themesLoadedData[I];
        }
        V(this, u).add({ ...J, newStartPoint: X, theme: (n == null ? void 0 : n.systemName) || m, meshName: (n == null ? void 0 : n.meshName) || L, colorSuffix: D });
      }
      X = !1;
    }
    Y ? Object.assign(this.rollGroupData[R].rolls, G) : (Z.rolls = G, Z.id = R, this.rollGroupData[R] = Z, ++S(this, o)._);
  });
}, w = new WeakSet(), M = function(b) {
  const l = this.rollGroupData[b], d = Object.values(l.rolls).map(({ collectionId: Z, id: m, meshName: W, ...G }) => G);
  let X = d.reduce((Z, m) => {
    const W = isNaN(m.value) ? 0 : m.value;
    return Z + W;
  }, 0);
  return X += l.modifier ? l.modifier : 0, { value: X, rollsArray: d };
};
class T {
  constructor(b) {
    Object.assign(this, b), this.rolls = b.rolls || [], this.completedRolls = 0;
    const l = this;
    this.promise = new Promise((d, X) => {
      l.resolve = d, l.reject = X;
    });
  }
}
export {
  Yl as d,
  Jl as default
};
