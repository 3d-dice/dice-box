...
let canvas, config, engine, scene, camera, diceBox, lights, physicsWorkerPort, dieCache = {}, count = 0, sleeperCount = 0, dieRollTimer = []
let diceBufferView = new Float32Array(8000)
...
const renderLoop = () => {
  // if no dice awake then stop the render loop and save some CPU power
...
const add = async (options) => {
  ...
  if(options.sides === 100) {
    ...
    newDie.d10Instance = await Dice.loadDie({...options, sides: 10, id: newDie.id + 10000}).then( response =>  {
