...
import { lightsOptionsType, pointedType } from '../types'
import { PointLight } from '@babylonjs/core/Lights/pointLight'
import { ShadowGenerator } from '@babylonjs/core/Lights/Shadows/shadowGenerator'
import { Vector3 } from '@babylonjs/core/Maths/math.vector'

const createPointLights = ( options: lightsOptionsType ) => {
  const { enableShadows = true, scene } = options

  const p_light1: pointedType = new PointLight("PointLight1", new Vector3(-10,35,10), scene)
  p_light1.intensity = .6
  
  const p_light2: pointedType = new PointLight("PointLight2", new Vector3(11,35,0), scene)
